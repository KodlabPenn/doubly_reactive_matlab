...
%
% Function:
%   LF = localfreespaceLIDAR2D(X, R, LIDAR, RobotRadius)
%
% Input:
...
function LF = localfreespaceLIDAR2D(X, R, LIDAR, RobotRadius)
% Author(s): 
...
% Modified range data defining the local free space using the separating hyperplane
% between each data point and the robot body
R = 0.5*(R - RobotRadius);

% Compute local workspace 
LF = (0.5*(LIDAR.Range-RobotRadius))*[-1 -1; -1 1; 1 1; 1 -1]; % Initialize the local workspace with the minimum square that respect the LIDAR sensing range 
...
for k = Imin
    if isempty(LF)
        ...
        [xi, yi] = polyxhplane(LF(:,1),LF(:,2), m, n);
        if sum(isinf(xi))
            test = 0;
...
[xi, yi] = polybool('intersection',LF(:,1), LF(:,2), LocalFootprint(:,1), LocalFootprint(:,2));
LF = [xi+X(1), yi+X(2)];
